-- ================================================================
-- 07_kolom_consent.sql
--
-- TUJUAN: menyimpan bukti persetujuan pelindungan data pribadi pada
-- setiap baris skrining.
--
-- DASAR HUKUM
-- -----------
-- UU No. 27 Tahun 2022 tentang Pelindungan Data Pribadi. Pasal 4
-- menempatkan "data dan informasi kesehatan" sebagai data pribadi
-- SPESIFIK, yang pemrosesannya memerlukan persetujuan eksplisit.
--
-- Memiliki halaman kebijakan privasi TIDAK CUKUP. Yang diperlukan
-- adalah kemampuan membuktikan bahwa pasien tertentu menyetujui isi
-- tertentu pada tanggal tertentu. Itulah gunanya ketiga kolom di bawah.
--
-- MENGAPA consent_version DIPERLUKAN
-- Bila isi kebijakan berubah — misalnya penerima data bertambah — tanpa
-- kolom versi tidak ada cara mengetahui pasien mana yang menyetujui isi
-- yang mana. Versi diambil dari VERSI_KEBIJAKAN di
-- src/constants/kebijakanPrivasi.js.
--
-- MENGAPA consent_wali DIPERLUKAN
-- Persetujuan bagi anak diberikan orang tua atau wali. Untuk pasien di
-- bawah 18 tahun, aplikasi meminta konfirmasi terpisah di formulir
-- identitas, dan hasilnya dicatat di sini.
--
-- BARIS LAMA
-- 96 baris yang sudah ada dikumpulkan SEBELUM gerbang persetujuan
-- dibuat, sehingga ketiga kolom ini akan NULL untuk baris tersebut.
-- Itu disengaja dan TIDAK boleh diisi mundur dengan nilai palsu —
-- memalsukan bukti persetujuan lebih buruk daripada mengakui bahwa
-- persetujuannya tidak pernah tercatat. Lihat catatan di bagian bawah
-- untuk penanganannya.
--
-- YANG TIDAK DILAKUKAN
-- --------------------
--  - TIDAK menghapus atau mengubah data yang sudah ada.
--  - Hanya menambah kolom; nilai baris lama tetap NULL.
--
-- URUTAN PENERAPAN
-- Jalankan berkas ini SEBELUM men-deploy kode aplikasi yang mengirim
-- consent_at / consent_version / consent_wali. Bila kode di-deploy
-- lebih dahulu, field tambahan itu diabaikan oleh RPC lama dan
-- persetujuan tidak tercatat.
-- ================================================================
...
ALTER TABLE public.screenings
  ...
  ADD COLUMN IF NOT EXISTS consent_wali    BOOLEAN;

COMMENT ON COLUMN public.screenings.consent_at IS
  'Waktu pasien menyetujui pemberitahuan pelindungan data pribadi. '
  'NULL pada baris yang dikumpulkan sebelum gerbang persetujuan ada. '
  'JANGAN diisi mundur.';

COMMENT ON COLUMN public.screenings.consent_version IS
  'Versi kebijakan privasi yang disetujui, dari VERSI_KEBIJAKAN di '
  'src/constants/kebijakanPrivasi.js. Diperlukan agar dapat dibuktikan '
  'isi mana yang disetujui bila kebijakan berubah.';

COMMENT ON COLUMN public.screenings.consent_wali IS
  'TRUE bila persetujuan diberikan orang tua atau wali, untuk pasien '
  'di bawah 18 tahun (UU 27/2022).';

-- Indeks untuk audit: mencari baris tanpa bukti persetujuan.
CREATE INDEX IF NOT EXISTS idx_screenings_tanpa_consent
  ON public.screenings (tanggal_skrining)
  WHERE consent_at IS NULL;


-- ── LANGKAH 2: RPC menyimpan bukti persetujuan ───────────────────
--
-- Fungsi diganti seluruhnya agar kolom consent ikut ditulis. Seluruh
-- logika skoring dipertahankan apa adanya — ambang WAJIB tetap sama
-- dengan src/utils/skoring.js dan dijaga tests/kontrak-jawaban.test.js.
...
BEGIN
  ...
  -- ========== EPDS ==========
  -- Juknis KJ.02.05/B.III/92/2025 hal. 11. Hanya item 10 bernilai 3
  -- ("Ya, agak sering") yang mengeskalasi risiko. Ambang ini WAJIB sama
  -- dengan NILAI_ESKALASI_E10 di src/utils/skoring.js.
  ...
  INSERT INTO screenings (
    ...
    consent_at, consent_version, consent_wali
  ) VALUES (
    ...
    v_kesimpulan_klinis,
    v_rekomendasi,
    -- Bukti persetujuan. NULL bila client versi lama yang tidak
    -- mengirimkannya — jangan diberi nilai bawaan palsu.
    (payload_data->>'consent_at')::TIMESTAMPTZ,
    payload_data->>'consent_version',
    (payload_data->>'consent_wali')::BOOLEAN
  );
END;
$function$;

-- Hak akses ditulis eksplisit, tidak bergantung pada default privilege
-- Supabase yang dapat berubah.
...
-- (a) Ketiga kolom sudah ada.
SELECT column_name, data_type, is_nullable
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'screenings'
  AND column_name IN ('consent_at', 'consent_version', 'consent_wali')
ORDER BY column_name;

-- (b) RPC sudah menulis kolom consent.
SELECT
  CASE
    WHEN pg_get_functiondef(p.oid) LIKE '%consent_version%' THEN 'SUDAH menyimpan consent'
    ELSE 'BELUM — masih versi lama'
  END AS status_rpc
FROM pg_proc p
JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'public' AND p.proname = 'simpan_skrining';

-- (c) search_path tetap terpasang setelah fungsi diganti.
SELECT p.proname, p.prosecdef AS security_definer, p.proconfig
FROM pg_proc p
JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'public' AND p.proname = 'simpan_skrining';

-- (d) Baris tanpa bukti persetujuan. Sebelum deploy client, seluruh
--     96 baris lama akan muncul di sini — itu wajar.
SELECT
  COUNT(*)                                        AS total,
  COUNT(*) FILTER (WHERE consent_at IS NULL)      AS tanpa_bukti_consent,
  COUNT(*) FILTER (WHERE consent_at IS NOT NULL)  AS dengan_bukti_consent
...
-- (f) Uji fungsional: instrumen sengaja tidak dikenali agar fungsi
--     menolak SEBELUM INSERT, sehingga tidak ada data uji masuk.
--     Harus memunculkan 'Instrumen tidak dikenali'.
-- SELECT public.simpan_skrining('{"instrumen":"UJI_TOLAK","jawaban":[]}'::JSONB);


-- ================================================================
-- MENANGANI 96 BARIS TANPA BUKTI PERSETUJUAN
-- ================================================================
-- Baris tersebut dikumpulkan sebelum gerbang persetujuan ada, sehingga
-- consent_at-nya NULL. JANGAN mengisinya mundur — memalsukan bukti
-- persetujuan adalah pelanggaran yang lebih berat daripada mengakui
-- bahwa persetujuannya tidak tercatat.
--
-- Pilihan penanganan yang sah, keputusan ada pada Puskesmas:
--
--  1. BIARKAN dengan catatan. Data tetap dipakai berdasarkan alasan
--     pelaksanaan tugas pelayanan publik, dan ketiadaan bukti
--     persetujuan didokumentasikan sebagai temuan yang sudah diperbaiki
--     ke depan. Paling sederhana dan jujur.
--
--  2. MINTA PERSETUJUAN ULANG pada kunjungan berikutnya. Paling patuh,
--     tetapi memerlukan tenaga di lapangan.
--
--  3. Bila Dinas Kesehatan atau auditor mempersyaratkan bukti untuk
--     seluruh baris, data tanpa persetujuan sebaiknya TIDAK diekspor
--     sampai persetujuannya diperoleh:
--
--       SELECT * FROM public.screenings WHERE consent_at IS NOT NULL;
--
-- Yang terpenting: keputusan ini dicatat, bukan didiamkan.
-- ================================================================
