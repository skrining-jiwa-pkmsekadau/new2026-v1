-- ================================================================
-- 08_idempotensi_skrining.sql
--
-- Menjamin retry dengan submission_id yang sama tidak membuat baris
-- kedua. Jalankan dan verifikasi file ini SEBELUM deploy client baru.
-- Baris lama tetap memiliki submission_id NULL dan tidak diubah.
-- ================================================================
...
DROP INDEX IF EXISTS public.idx_screenings_submission_id;
CREATE UNIQUE INDEX idx_screenings_submission_id
  ON public.screenings (submission_id)
...
DROP FUNCTION public.simpan_skrining(JSONB);
...
BEGIN
  ...
  INSERT INTO screenings (
    ...
    consent_at, consent_version, consent_wali, submission_id
  ) VALUES (
...
-- ================================================================
-- VERIFIKASI
-- Hanya hasil "SIAP deploy client" yang boleh diterima.
-- ================================================================
WITH fungsi AS (
...
SELECT CASE WHEN
  ...
  AND NOT EXISTS (
    ...
    WHERE routine_schema = 'public'
      AND routine_name = 'simpan_skrining'
      AND grantee = 'PUBLIC'
...
THEN 'SIAP deploy client'
ELSE 'BELUM — jangan deploy client'
END AS status_idempotensi;
